class Parser:
    ...
    def parseFile(self, filename):
        ...
        self._flag = True # Je li parsiranje uspjesno?
        self._func = ""
        self._labels = {} #sprema label svaki put kad je goto/ifgoto deklariran
        self._extra = {} #ako nam neki label dolazi prije goto npr END
        ...
        if self._labels:
            for label in self._labels:
                Parser._error("Label", self._labels[label], "Label \"" + label + "\" is referecned but does not exist.")
            return
    ...
    def _push(self, src, loc, n):
        if not loc.isnumeric():
            ...
            if int(loc) < 0:
                self._flag = False
                Parser._error("Push", n, "Location cannot have negative value " + loc + ".")
                return ""
            elif src == "constant":
                l = "@" + str(loc) + "\nD=A\n"
            elif src == "local":
    ...
    def _goto(self, lab, n):
        ...
        if label in self._labels:
            self._flag = False
            Parser._error("goto", n, "Label \"" + label + "\" already exists.")
            return ""
        self._labels[label] =  n
        if label in self._extra:
            self._extra.pop(label)
            self._labels.pop(label)
        return "@" + self._func + "$" + lab + "\n0;JMP"
    ...
    def _ifgoto(self, lab, n):
        ...
        if label in self._labels:
            self._flag = False
            Parser._error("ifgoto", n, "Label \"" + label + "\" already exists.")
            return ""
        self._labels[label] =  n
        if label in self._extra:
            self._extra.pop(label)
